-- 007: Add is_current flag for opphevede lover
-- When Lovdata publishes gjeldende-lover.tar.bz2, it only contains current laws.
-- Our sync does upsert, so replaced/repealed laws stay in the database forever.
-- This migration adds is_current to distinguish gjeldende from opphevede.
-- Depends on: 005_legal_area_in_search_results.sql (search function signatures)

-- =============================================================================
-- 1. New column + index
-- =============================================================================

ALTER TABLE lovdata_documents ADD COLUMN IF NOT EXISTS is_current BOOLEAN DEFAULT TRUE;
CREATE INDEX IF NOT EXISTS idx_lovdata_documents_is_current ON lovdata_documents(is_current);

-- =============================================================================
-- 2. RPC function: mark_non_current_docs
--    Called by sync after processing each dataset to mark documents
--    not present in the latest tar.bz2 as non-current.
-- =============================================================================

CREATE OR REPLACE FUNCTION mark_non_current_docs(p_doc_type TEXT, p_current_ids TEXT[])
RETURNS INTEGER
LANGUAGE plpgsql
...
DECLARE cnt INTEGER;
BEGIN
    -- Mark all of this type NOT in the list as non-current
    UPDATE lovdata_documents
    SET is_current = FALSE
    WHERE doc_type = p_doc_type
      AND is_current = TRUE
      AND dok_id != ALL(p_current_ids);
    GET DIAGNOSTICS cnt = ROW_COUNT;

    -- Mark all in the list as current (handles "resurrected" docs)
    UPDATE lovdata_documents
    SET is_current = TRUE
    WHERE doc_type = p_doc_type
      AND is_current = FALSE
      AND dok_id = ANY(p_current_ids);
...
-- =============================================================================
-- 3. Drop old function signatures to avoid overload conflicts
-- =============================================================================
...
-- =============================================================================
-- 4. Updated search_lovdata_fast — adds include_non_current filter
-- =============================================================================
...
CREATE OR REPLACE FUNCTION search_lovdata_fast(
    ...
    include_non_current BOOLEAN DEFAULT FALSE
)
RETURNS TABLE (
    dok_id TEXT,
    section_id TEXT,
...
BEGIN
    ...
    WITH ranked AS (
        ...
        WHERE s.search_vector @@ websearch_to_tsquery('norwegian', query_text)
          ...
          AND (include_non_current OR COALESCE(d.is_current, TRUE) = TRUE)
...
-- =============================================================================
-- 5. Updated search_lovdata_hybrid — adds include_non_current filter
-- =============================================================================
...
BEGIN
    ...
    WITH
    filtered_docs AS (
        SELECT d.dok_id, d.short_title, d.doc_type, d.ministry, d.based_on, d.legal_area, d.is_current
    ...
    vector_search AS (
        ...
        WHERE s.embedding IS NOT NULL
          AND (doc_type_filter IS NULL AND ministry_filter IS NULL AND NOT exclude_amendments AND legal_area_filter IS NULL AND include_non_current
...
-- =============================================================================
-- 6. Updated find_similar_law — prioritize is_current in results
-- =============================================================================

CREATE OR REPLACE FUNCTION find_similar_law(
    search_term TEXT,
...
AS $$
    SELECT
        ...
        d.short_title,
        d.doc_type,
        similarity(d.short_title, search_term) AS similarity
    FROM lovdata_documents d
    WHERE similarity(d.short_title, search_term) > similarity_threshold
    ORDER BY COALESCE(d.is_current, TRUE) DESC, similarity DESC
    LIMIT max_results;
$$;
