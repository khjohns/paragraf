...
Kolonnene populeres under sync. Metadata-dekning etter sync (2026-02-09): ministry 100%, based_on 96% (forskrifter), legal_area 18%.

**Droppede kolonner (migration 004):** `keywords`, `language`, `date_end` ble droppet fordi de var 0% populert. Disse finnes i Lovdatas XML-formatspesifikasjon, men det er uverifisert om public API faktisk inkluderer dem — 0% kan skyldes at API-et utelater dem, eller en parser-bug. Ved neste sync med nettverkstilgang bør man verifisere rå-XML for et sample av dokumenter.
